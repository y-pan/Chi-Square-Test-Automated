...
public class ChiSquare {
     ...
     */
    public static double getChiSquareCriticalValue(int[][] observed, double[][] expected){
    
        double result = 0.0;
        for(int i = 0; i< observed.length; i++){
            for(int j=0; j<observed[i].length; j++){
                double o = (double)observed[i][j];
                double e = expected[i][j];
                result += (e-o)*(e-o) * 1.0 /e;
            }
        }
        return result;
    }
     ...
     * @return p value, AKA: probability level, or α. Hint: 
     */
    public static double getPValue(double chiSqValue, int df) {
        double a, s;
        double e, c, z;

        if (chiSqValue <= 0.0 || df < 1) {
            return 1.0;
        }
        a = 0.5 * chiSqValue;
        boolean even = (df & 1) == 0;
        ...
        s = (even ? y : (2.0 * poz(-Math.sqrt(chiSqValue))));
        if (df > 2) {
            chiSqValue = 0.5 * (df - 1.0);
            z = (even ? 1.0 : 0.5);
            if (a > MAX_X) {
